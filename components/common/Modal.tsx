...
const Modal = ({ children, isOpen, onClose, centerContent = true, className, customPosition, isFullScreen = true, noPadding, header, leftHeaderClass }: ModalProps) => {
    return (
        <div className={`absolute ${isOpen ? "h-svh" : "h-[-100px]"} ${isFullScreen ? "z-20" : ""} left-0 top-0 flex items-center justify-center w-full`}>
            ...
            <div className={`${isOpen ? `${centerContent ? "top-1/2 transform -translate-y-1/2" : customPosition}` : "top-[-1500px]"} w-full max-w-[90%] sm:max-w-[480px] max-h-[85svh] sm:max-h-[95svh] overflow-y-auto rounded-md transition-all duration-500 ease-in-out absolute dark:bg-gray-2B2C37 bg-white-FFFFFF ${className}`}>

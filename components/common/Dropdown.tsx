...
const Dropdown = ({ value, options, onChange }: DropdownProps) => {
    ...
    return (
        <div ref={dropdownRef} onClick={() => setShowOptions(!showOptions)} className={`cursor-pointer relative border border-gray-828FA33F ${showOptions ? " border-violet-635FC7" : ""} flex justify-between px-4 py-2 rounded items-center`}>
            ...
            {showOptions && options?.length ? (<div className="absolute w-full dark:bg-black-20212C bg-white-FFFFFF flex flex-col gap-1 left-0 top-11 rounded-lg p-2 max-h-[117px] overflow-y-auto">
